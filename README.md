...
> **当前状态：`0.2.0-rc.2`。** 这是公开预览候选版，不是稳定发行版。自动测试可以证明
...
johnrucnapier-sketch/ACGM-for-Codex --ref v0.2.0-rc.2 --json` 与 `codex plugin
...
`0.1.0-rc.2`、`0.1.0-rc.3`、`0.1.0-rc.4` 或 `0.2.0-rc.1`；此时 digest 会明确绑定
...
升级期间，已经打开的旧 task 仍会调用启动时记住的旧版本 Hook 路径。RC2 会在每一步保护这个
路径，并为所有已知、尚未内置保护的官方旧版本生成可校验的 fail-open bridge；因此截图里已
经失效的 RC4 task 也被覆盖。旧 task 不会因缺失 Stop Hook
进入模型循环，但完整 ACGM 要在重启 task 后才重新加载。新版本 Hook 命令自身也内置运行时
缺失保护，避免下一次升级重现同类自锁。这不是 Lite 模式，也不放宽正常运行时的治理规则。
RC2 还会严格识别 Codex 在 marketplace 更新后出现的“旧版本 cache + 新 ref”短暂重关联状态；
只有旧 cache、目标 checkout、scope、policy 与官方发布 pin 全部匹配才继续。被 RC1 中断的同类
状态必须先生成新的 RC2 digest，再自动执行 remove/add/plugin-add，不会要求用户手工修配置。
...
上面列出的四个精确官方候选版是自动插件升级例外。Quickstart 永远不会吸收、重置或搬运
...
仍完全匹配、而且来源是明确兼容的 RC2/RC3/RC4/0.2-RC1 project adapter 时，quickstart 才会在同一次
